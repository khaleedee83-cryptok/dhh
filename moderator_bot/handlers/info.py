...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Handles the /start command — only responds in private chat, and only to the owner."""
    message = update.effective_message
    chat    = update.effective_chat
    user    = update.effective_user
    if not message or not chat or not user:
        return
    # Only respond in private (DM / Saved Messages)
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.effective_message.reply_text(
        ...
        "/flood <count> — max messages before flood action\n"
        "/caps <0-100> — max uppercase ratio (e.g. 75 = 75%)\n"
